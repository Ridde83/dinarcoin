...
#if defined(NDEBUG)
# error "Dinarcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8a10a83512f7f28607b88bfc41bb027e0bf21c0bf9d149c4e174744d030b9184");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Dinarcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Dinarcoin Signed Message:\n";
...
{
    // Dinarcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Dinarcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Dinarcoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Dinarcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Dinarcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Dinarcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1c1a298bc2657277e9ddcde7d0af26fde589b375425a8f97ff5da3ecafced0ca");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "2021/04/18 - Ramadan Month - DinarCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0408fa21dc7c5d7cd0bb065146d30eb842c1ce057ef31650ee48bfb7681d33649aa732b6bea26c3e7e83cb960e77bc20cf6baed853cefd49a9f7429a9081fad75d") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1618762901;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085031909;

        if (fTestNet)
        {
            block.nTime    = 1618762840;
            block.nNonce   = 386083490;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x9d18dfdd9cbced5fc6664ee584f99d7153e69bd591a399c0e3ca22efa8c87860"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb4, 0xdc }; // Dinarcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DinarcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DinarcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DinarcoinMiner(CWallet *pwallet)
{
    printf("DinarcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dinarcoin-miner");
    ...
    try { loop {
        ...
        printf("Running DinarcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DinarcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DinarcoinMiner, pwallet));
}
